...
SET VERSIONE_WITH_TEXT=11.4.0.0-master
...
call "%BATCH_FOLDER%\build-deploy-FTLIBSERVICE-RELEASE" %ROOT_MOUNTPOINT% %ROOT_MOUNTPOINT_COMMON% "%PATH_MSBUILD%" >> %LOG_FILE% 2>> %LOG_FILE_ERR%

call "%BATCH_FOLDER%\build-deploy-RDP-RELEASE.bat" %ROOT_MOUNTPOINT% %ROOT_MOUNTPOINT_COMMON% "%PATH_MSBUILD%" >> %LOG_FILE% 2>> %LOG_FILE_ERR%

call "%BATCH_FOLDER%\build-deploy-TCPIP-RELEASE.bat" %ROOT_MOUNTPOINT% %ROOT_MOUNTPOINT_COMMON% "%PATH_MSBUILD%" >> %LOG_FILE% 2>> %LOG_FILE_ERR%

call "%BATCH_FOLDER%\build-deploy-FTSYSTEMSHUTDOWN-RELEASE.bat" %BATCH_FOLDER% %ROOT_MOUNTPOINT% %ROOT_MOUNTPOINT_COMMON% "%PATH_MSBUILD%" %FTSHUTDOWN_OUTPUT_FOLDER% >> %LOG_FILE% 2>> %LOG_FILE_ERR%

call "%BATCH_FOLDER%\build-TDK-DESIGN.bat" %ROOT_MOUNTPOINT% %PATH_MSBUILD% 1>> %LOG_FILE% 2>> %LOG_FILE_ERR%

call "%BATCH_FOLDER%\build-OVERLORD ver-NON-ufficiale.bat" %VERSIONE_WITH_TEXT% %VERSIONE% %ROOT_MOUNTPOINT% %VB6_OUTPUT% %STANDALONE% >> %LOG_FILE% 2>> %LOG_FILE_ERR%

call "%BATCH_FOLDER%\build-DEVICEDRIVER-RELEASE.bat" %ROOT_MOUNTPOINT% %ROOT_MOUNTPOINT_COMMON% >> %LOG_FILE% 2>> %LOG_FILE_ERR%
